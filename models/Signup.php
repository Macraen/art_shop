...
class Signup extends Model
{
    ...
    public function rules()
    {
        return [
            ...
            [['firstname','surename','email','date','city','password','confpassword'],'required','message' => "Це поле бов'язкове для заповнювання."],
            ['email','email','message' => "Введіть коректну адресу."],
            ['email','unique','targetClass'=>'app\models\User','message' => "Ця пошта вже використовується."],
    ...
    public function sendConfirmationLink(){
        $confirmationLinkUrl = Url::to(['/site/confirmemail', 'email'=>$this->email, 'code'=>$this->code],'https');
    ...
    public function signup()
    {
        if ($this->validate()){
            ...
            $user->code = $this->code;
            $user->active = 0;
            $user->photo = '/webroot/uploads/unnamed.png';
